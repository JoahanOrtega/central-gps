...
import type { CreatePoiGroupPayload } from "../../types/poi.types"
...
export const NewPoiGroupModal = ({
  open,
  onOpenChange,
  onCreated,
}: NewPoiGroupModalProps) => {
  ...
  const handleChange = (
    ...
    if (name === "id_cliente") {
      setForm((prev) => ({
        ...prev,
        id_cliente: value ? Number(value) : null,
      }))
      return
    }
  ...
  const handleSubmit = async () => {
    try {
      ...
      const message =
        error instanceof Error ? error.message : "No fue posible guardar el grupo"
      setError(message)
    } finally {
      setIsLoading(false)
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        className="w-[95vw] max-w-[720px] max-h-[90vh] overflow-hidden p-0"
      >
