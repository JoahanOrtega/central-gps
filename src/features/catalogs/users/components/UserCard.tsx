...
export const UserCard = ({
    ...
    // Construir items del KebabMenu según permisos
    const menuItems = [
        canEdit && {
            id: "edit",
            label: "Editar",
        ...
        showInhabilitar && {
            ...
            label: "Inhabilitar",
            icon: UserMinus,
            // "warning" (ámbar) porque inhabilitar NO es eliminar — es reversible.
            // Diferencia visual para que el usuario entienda que es menos
            // destructivo que un delete.
            variant: "warning" as const,
            onClick: () => onInhabilitar?.(user),
        },
    ...
    return (
        <article className="relative rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition-shadow hover:shadow-md">
            <div className="flex items-start justify-between gap-3">
                ...
                {/* Bloque derecho: kebab menu compartido */}
                {menuItems.length > 0 && (
                    <KebabMenu items={menuItems} entityName={user.nombre} />
                )}
            </div>
