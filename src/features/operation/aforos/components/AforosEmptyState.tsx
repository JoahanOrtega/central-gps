...
interface AforosEmptyStateProps {
  search: string;
  onClearSearch: () => void;
  puedeCrear: boolean;
  onAddClick: () => void;
...
export const AforosEmptyState = ({ search, onClearSearch, puedeCrear, onAddClick }: AforosEmptyStateProps) => {
  return (
    <div className="flex flex-col items-center justify-center py-12 px-4 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50/50 text-center max-w-xl mx-auto my-8">
      ...
      <h3 className="text-base font-semibold text-slate-800 mb-1">
        {search ? "No se encontraron resultados" : "No hay aforos registrados"}
      </h3>
      <p className="text-sm text-slate-500 max-w-sm mb-6">
        {search 
          ? `No encontramos coincidencias para "${search}".` 
          : "Aún no hay aforos. Puedes agregar uno manualmente o usar el botón de Subir Archivo arriba."}
      </p>
      {puedeCrear && !search && (
        <button
          onClick={onAddClick}
